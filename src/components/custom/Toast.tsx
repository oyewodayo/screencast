  ...
  return (
    <div
      className={`flex items-start gap-2.5 max-w-sm px-3.5 py-3 rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.18)] ring-1 backdrop-blur-xl transition-all duration-200 ease-out ${
        isError ? "bg-red-50/95 ring-red-200" : "bg-white/95 ring-black/[0.06]"
    ...
    >
      ...
      <div className={`flex-1 text-sm break-words ${isError ? "text-red-700" : "text-neutral-700"}`}>{message}</div>
      <button
        type="button"
        title="Dismiss"
        onClick={onDismiss}
        className={`shrink-0 rounded-full p-0.5 hover:bg-black/5 ${isError ? "text-red-400" : "text-neutral-400"}`}
      >
        <IoClose size={15} />
      </button>
    </div>
