...
fn scan_directory(root: &Path, dir: &Path, result: &mut HashMap<String, Vec<FileEntry>>) {
    ...
    for entry in entries.flatten() {
        ...
        if entry_path.is_file() {
            ...
            // services/boards.rs), doc projects (Docs, see services/docs.rs), and whiteboard
            // projects (Whiteboards, see services/whiteboards.rs) all live here but must never
            // surface in the normal file list — that's the whole point of each being "hidden"
            // rather than just another folder.
